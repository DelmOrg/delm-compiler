@preprocessor typescript

@{%

const keywords = [
  "port",
  "module",
  "exposing",
  "import",
  "as",
  "(..)",
  "type",
  "alias",
  "case",
  "of",
  "_",
  "->",
  "let",
  "in",
];

%}

DECLARATION ->  "type" __ ID_UNWRAPPED __ "=" __ S_STATEMENT

			{% ([type, , dcl, , eq, , stm]) => { return { type: "TYPE_DECLARATION", left: dcl, right: stm } } %}


EXPRESSIONS -> EXPRESSIONS __ (IDENTIFIER {% ([id]) => id %} | LITERAL {% ([id]) => id[0] %})

					{% ([es, _, v]) => [v, ...es] %}

				| null


S_STATEMENT -> S_STATEMENT __ "|" __ ID_UNWRAPPED (__ STATEMENT | null)

				{% ([stms, ,,, id, [, stm]]) => { return [...stms, { id, statements: stm }] } %}

			| ID_UNWRAPPED (__ STATEMENT | null)

				{% ([id, [, stm]]) => { return [{ id, statements: stm }] } %}


STATEMENT -> LITERAL
				| "(" _ STATEMENT _ ")"

					{% ([p1, , v, , p2]) => { return v } %}

				| IDENTIFIER EXPRESSIONS
		
					{% ([id, params]) => { return params.length > 0 ? [{ id, statements: params.reverse() }] : [{ id, statements: [] }] } %}


LITERAL -> "(" TUPLE ")"

			{% ([, v, ]) => { return [{ type: 'TUPLE', value: v }] } %}

		| "(" ")"

			{% () => { return [{ type: 'TUPLE', value: null }] } %}


TUPLE -> __ STATEMENT _ "," __ S_STATEMENT _ "," __ S_STATEMENT __

			{% ([_, [st1], , comma1, __, [st2], , comma2, ___, [st3]]) => [st1, st2, st3] %}

		|  __ S_STATEMENT _ "," __ S_STATEMENT __
		
			{% ([_, [st1], __, comma1, , [st2]]) => [st1, st2] %}

IDENTIFIER -> IDENTIFIER "." ID

			{% ([identf, dot, [id]]) => { return { type: 'IDENTIFIER', value: identf.value + dot + id.join("") } } %}

		| ID

			{% ([n]) => { return { type: 'IDENTIFIER', value: n[0].join("") } } %}


ID_UNWRAPPED -> ID

			{% ([n]) => { return { type: 'IDENTIFIER', value: n[0].join("") } } %}


ID -> [a-zA-Z]:+

			{% (n, _, reject) => { if ( keywords.includes(n[0].join("")) ) { return reject } else { return n } } %}

		| [a-zA-Z]:+ [0-9]:+ (ID {% ([n]) => n %} | null)

			{% ([n1, n2, [n3]]) => { return n3?.[0] ? [[...n1, ...n2, ...n3]] : [[...n1, ...n2]]} %}


_ -> " ":?  {% () => null %}
__ -> " " 	{% () => null %}
